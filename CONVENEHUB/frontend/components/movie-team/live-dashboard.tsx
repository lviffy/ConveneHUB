...
export default function LiveDashboard({
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch(`/api/movie-team/events/${eventId}/stats`, {
        cache: 'no-store',
        headers: {
  ...
  const fetchNotes = async () => {
    try {
      const response = await fetch(`/api/movie-team/events/${eventId}/notes`);
      if (response.ok) {
        const data = await response.json();
        setNotes(data.notes || '');
      }
  ...
  const fetchCheckedInUsers = async () => {
    try {
      ...
      const response = await fetch(`/api/movie-team/events/${eventId}/checked-in-users`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    try {
      const response = await fetch(`/api/movie-team/events/${eventId}/status`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
