...
const { normalizeAuthRole, requireAuth } = require('../middlewares/auth.middleware');
...
function resolveTenantId(role, tenantId) {
...
function sanitizeAuthIntent(intent) {
  if (intent === 'signup') return 'signup';
  return 'signin';
}

function oauthErrorRedirect(code, details) {
...
function resolveOtpType(inputType, shouldCreateUser) {
...
authRouter.get('/google/callback', async (req, res) => {
  ...
  let oauthState;
  try {
    oauthState = jwt.verify(stateToken, env.JWT_ACCESS_SECRET);
  } catch {
...
authRouter.post('/login', async (req, res) => {
  ...
  const normalizedRole2 = normalizeRole(user.role);
  const payload = { sub: String(user._id), role: normalizedRole2, tenantId: user.tenantId };
  const accessToken = signAccessToken(payload);
  const refreshToken = signRefreshToken(payload);
...
async function sendOtpHandler(req, res, forcedType) {
...
authRouter.post('/verify-otp', async (req, res) => {
  ...
  const effectiveType = parsed.data.type || user.otpType || 'signup';
  ...
  const normalizedRole3 = normalizeRole(user.role);
  const payload = { sub: String(user._id), role: normalizedRole3, tenantId: user.tenantId };
  const accessToken = signAccessToken(payload);
  const refreshToken = signRefreshToken(payload);
...
authRouter.post('/complete-profile', requireAuth, async (req, res) => {
  ...
  return res.json({ success: true, user: toFrontendUser(user) });
});
...
authRouter.get('/me', requireAuth, async (req, res) => {
  ...
  const normalizedRole4 = normalizeRole(user.role);
  const accessToken = signAccessToken({
    sub: String(user._id),
    role: normalizedRole4,
    tenantId: user.tenantId,
  });
