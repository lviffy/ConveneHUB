...
const { z } = require('zod');
const { syncTenantRecord } = require('../utils/tenants');
...
function fromFrontendRole(role) {
...
adminRouter.get('/financial-summary', requireAuth, requireRole('admin'), async (_req, res) => {
  ...
  if (events.length === 0) {
    return res.json({
      ...
      events: [],
      summary: {
        total_events: 0, total_tickets_sold: 0, total_gross_revenue: 0,
        total_processing_fees: 0, total_platform_commission: 0, total_net_payout: 0,
      },
      fee_structure: { processing_fee_percentage: PROCESSING_FEE_PERCENTAGE, platform_commission_note: 'Commission percentage varies per event' },
    });
  }
  ...
  const eventsPayload = events.map((event) => {
    ...
    return {
      event_id: eventId, title: event.title, date_time: event.dateTime, venue_name: event.venue,
      city: event.city || '', status: mapEventStatus(event.status),
      settlement_status: event.settlementStatus || 'pending',
      settlement_details: event.settlementDetails || null,
      financial_summary: {
        total_bookings: totalBookings, total_tickets_sold: totalTicketsSold, free_bookings: 0, paid_bookings: totalBookings,
        gross_revenue: toMoney(grossRevenue), processing_fees: toMoney(processingFees),
        processing_fee_percentage: PROCESSING_FEE_PERCENTAGE, platform_commission: toMoney(platformCommission),
        platform_commission_percentage: PLATFORM_COMMISSION_PERCENTAGE, net_payout_to_movie_team: toMoney(netPayout),
      },
      bookings: eventBookings.map((booking) => ({
        booking_id: String(booking._id), tickets_count: booking.ticketsCount, total_amount: toMoney(booking.amount),
        booking_status: booking.bookingStatus, payment_required: false, payment_status: booking.paymentStatus || 'not_required',
        booked_at: booking.createdAt,
      })),
    };
  });

  const summary = eventsPayload.reduce((acc, event) => {
    acc.total_events += 1;
    ...
    return acc;
  }, { total_events: 0, total_tickets_sold: 0, total_gross_revenue: 0, total_processing_fees: 0, total_platform_commission: 0, total_net_payout: 0 });

  return res.json({
    success: true, events: eventsPayload,
    summary: {
      total_events: summary.total_events, total_tickets_sold: summary.total_tickets_sold,
      total_gross_revenue: toMoney(summary.total_gross_revenue), total_processing_fees: toMoney(summary.total_processing_fees),
      total_platform_commission: toMoney(summary.total_platform_commission), total_net_payout: toMoney(summary.total_net_payout),
    },
    fee_structure: { processing_fee_percentage: PROCESSING_FEE_PERCENTAGE, platform_commission_note: 'Commission percentage varies per event' },
  });
});
...
adminRouter.get('/reconciliation', requireAuth, requireRole('admin'), async (_req, res) => {
  const events = await EventModel.find({ status: { $in: ['published', 'checkin_open', 'in_progress', 'ended', 'closed'] } }).sort({ dateTime: -1 }).lean();
  const eventIds = events.map((event) => String(event._id));
  ...
  const eventsPayload = events.map((event) => {
    ...
    if (pendingPayments > 0) { reconciliationStatus = 'pending'; discrepancyReason = 'Pending payments to be resolved'; }
    if (totalBookings > 0 && noShows > totalBookings * 0.2) { reconciliationStatus = 'discrepancy'; discrepancyReason = 'High no-show rate (>20%)'; }

    return {
      event_id: eventId, event_title: event.title, event_date: event.dateTime,
      total_bookings: totalBookings, total_checkins: totalCheckins, no_shows: noShows,
      paid_bookings: paidBookings, free_bookings: freeBookings, pending_payments: pendingPayments,
      failed_payments: failedPayments, gross_revenue: toMoney(grossRevenue),
      reconciliation_status: reconciliationStatus, discrepancy_reason: discrepancyReason,
  ...
  if (highNoShowEvents > 0) { summary.discrepancies.push({ type: 'high_no_show', count: highNoShowEvents, description: 'High no-show rate detected' }); }
  const pendingPayments2 = eventsPayload.reduce((sum, event) => sum + event.pending_payments, 0);
  if (pendingPayments2 > 0) { summary.discrepancies.push({ type: 'pending_payments', count: pendingPayments2, description: 'Pending payments awaiting verification' }); }
...
adminRouter.post('/settlements', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const transaction_reference = typeof req.body?.transaction_reference === 'string' ? req.body.transaction_reference.trim() : '';
  ...
  event.settlementDetails = {
    transaction_reference, transfer_date, payment_method,
    notes: notes || undefined, settled_by: req.user?.sub, settled_at: new Date(),
  };
  await event.save();
...
adminRouter.post('/financial-summary/email', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (!recipient) { return res.status(400).json({ success: false, error: 'recipient_email is required' }); }
...
adminRouter.post('/settlements/email', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (!eventId || !recipient) { return res.status(400).json({ success: false, error: 'event_id and movie_team_email are required' }); }
...
adminRouter.get('/events/:eventId/export-bookings', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  const bookings = await BookingModel.find({ eventId, bookingStatus: { $ne: 'cancelled' } }).sort({ createdAt: -1 }).lean();
  ...
  const payload = bookings.map((booking) => {
    ...
    return {
      booking_code: booking.bookingCode, booking_id: String(booking._id),
      user_name: user?.fullName || 'Attendee', user_email: user?.email || '', user_phone: user?.phone || '',
      tickets_count: booking.ticketsCount, total_amount: toMoney(booking.amount),
      booking_status: booking.bookingStatus, booked_at: booking.createdAt,
      checked_in: Boolean(checkedTicket), checked_in_at: checkedTicket?.checkedInAt,
...
adminRouter.get('/events/:eventId/export-checkins', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  if (checkins.length === 0) { return res.json({ success: true, checkIns: [] }); }
  ...
  const payload = checkins.map((checkin) => {
    ...
    return {
      booking_code: booking?.bookingCode || '', booking_id: booking ? String(booking._id) : '',
      user_name: user?.fullName || 'Attendee', user_email: user?.email || '', user_phone: user?.phone || '',
      tickets_count: booking?.ticketsCount || 1, checked_in_at: checkin.createdAt,
      checked_in_by_name: scanner?.fullName || 'Staff', method: checkin.method,
      booked_at: booking?.createdAt || '',
    };
  });
