...
function resolveApiTarget(url) {
  ...
  const normalizedApiPath = parsed.pathname.startsWith("/api/v1/") ? parsed.pathname : parsed.pathname.replace(/^\/api\//, "/api/v1/");
...
if (typeof window !== "undefined") {
  ...
  window.fetch = async (input, init) => {
    ...
    const {
      isApiCall,
      targetUrl
    } = resolveApiTarget(url);
    if (!isApiCall) {
      return originalFetch(input, init);
    }
    const requestInput = typeof input === "string" || input instanceof URL ? targetUrl : new Request(targetUrl, input);
