...
import { Calendar, MapPin, Users, Eye, Edit, Trash2, Download, FileText, RefreshCw } from "lucide-react";
...
export default function EventsList() {
  ...
  const fetchEvents = async () => {
    try {
      ...
      const eventsWithRealCount = await Promise.all((data || []).map(async event => {
