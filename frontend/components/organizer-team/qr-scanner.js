...
export default function QRScanner({
  ...
  const initializeCamera = async () => {
    try {
      ...
      const backCameraIndex = cameras.findIndex(c => c.label.toLowerCase().includes("back") || c.label.toLowerCase().includes("environment"));
  ...
  const handleScanResult = data => {
    ...
    setScanResult({
      success: isValid,
      message: isValid ? `Check-in successful! Booking: ${data.substring(0, 8)}...` : "Invalid QR code"
    });
