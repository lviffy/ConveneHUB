...
import { Calendar, MapPin, Clock, Users, ArrowLeft, Shield, Film, CheckCircle2, Info, Ticket, AlertCircle, X } from "lucide-react";
...
const VenueMap = dynamic(() => import("@/components/events/venue-map").then(mod => ({
...
export default function EventBookingPage({
  ...
  const persistReferralForEvent = useCallback(code => {
  ...
  const checkUserBooking = useCallback(async userId => {
    try {
      const {
        data,
        error
          ...
          checked_in,
          qr_nonce
        `).eq("event_id", eventId).eq("user_id", userId).neq("booking_status", "cancelled").in("payment_status", ["NOT_REQUIRED", "SUCCESSFUL", "PAID"]) // Only show confirmed bookings
      .maybeSingle();
      if (error) {}

      // Only set as existing booking if payment was successful or not required
      const validBooking = data && (data.payment_status === "NOT_REQUIRED" || data.payment_status === "SUCCESSFUL" || data.payment_status === "PAID");
  ...
  const selectedTier = availableTiers.find(tier => tier.name === selectedTierName) || availableTiers[0] || null;
  const selectedTierPrice = selectedTier ? Number(selectedTier.price || 0) : Number(event?.ticket_price || 0);
  const maxTicketsForSelection = Math.max(1, Math.min(MAX_TICKETS_PER_USER, event?.actualRemaining || 0, selectedTier ? selectedTier.remaining : event?.actualRemaining || 0));
