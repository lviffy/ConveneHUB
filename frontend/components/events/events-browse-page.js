...
import { Calendar, MapPin, Clock, Users, Search, X, SlidersHorizontal, Grid3X3, LayoutList, Ticket, Sparkles, Filter, ArrowRight } from "lucide-react";
  ...
  const fetchEvents = useCallback(async () => {
    try {
      ...
      if (normalizedEvents.length > 0) {
        ...
        const eventCities = normalizedEvents.map(event => event.city).filter(Boolean).map(city => city.trim());
  ...
  const filteredEvents = useMemo(() => {
    return events.filter(event => {
      if (searchQuery) {
        ...
        const matchesSearch = event.title.toLowerCase().includes(query) || event.venue_name?.toLowerCase().includes(query) || event.city?.toLowerCase().includes(query) || event.description?.toLowerCase().includes(query);
