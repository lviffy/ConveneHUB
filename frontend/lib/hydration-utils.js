...
export const suppressKnownHydrationWarnings = () => {
  ...
  console.error = (...args) => {
    ...
    const knownExtensionAttributes = ["cz-shortcut-listen",
    ...
    if (messageText.includes("Extra attributes from the server") && knownExtensionAttributes.some(attr => messageText.includes(attr))) {
      // Suppress known browser-extension hydration warnings.
      return;
    }
    if (messageText.includes("Support for defaultProps will be removed from function components in a future major release") && (messageText.includes("XAxis") || messageText.includes("YAxis") || argsText.includes("recharts"))) {
