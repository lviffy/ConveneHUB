...
function mapBackendEvent(event) {
  ...
  const generalTier = tiers.find(tier => String(tier?.name || "").trim().toLowerCase() === "general") || firstTier;
  const vipTier = tiers.find(tier => String(tier?.name || "").trim().toLowerCase() === "vip") || null;
...
class QueryBuilder {
    ...
    if (this.operation === "select") {
      ...
      if (this.returnSingle) {
        if (filtered.length === 0) {
          return this.makeResult(this.allowEmptySingle ? null : null, this.allowEmptySingle ? null : {
    ...
    if (this.operation === "update") {
      ...
      if (!response.ok) {
        return this.makeResult(null, {
          message: payload?.message || payload?.error || "Failed to update profile"
        });
      }
      const user = mapBackendUser(payload.user || {});
  ...
  async runEvents() {
    if (this.operation === "select") {
      ...
      if (!response.ok) {
        return this.makeResult([], {
          message: payload?.message || payload?.error || "Failed to fetch events"
        });
      }
      const rows = (payload.events || []).map(event => mapBackendEvent(event));
      const filtered = this.applyFilters(rows);
      if (this.returnSingle) {
        if (filtered.length === 0) {
          return this.makeResult(this.allowEmptySingle ? null : null, this.allowEmptySingle ? null : {
    ...
    if (this.operation === "insert") {
      ...
      const hasVipTier = body?.vip_ticket_price !== undefined && body?.vip_ticket_price !== null && String(body?.vip_ticket_price).trim() !== "";
      ...
      if (!response.ok) {
        return this.makeResult(null, {
          message: payload?.message || payload?.error || "Failed to create event"
        });
      }
    ...
    if (this.operation === "update") {
      ...
      const hasTierPriceUpdate = this.payload?.ticket_price !== undefined || this.payload?.vip_ticket_price !== undefined;
      const generalPrice = Number(this.payload?.ticket_price ?? 0);
      const hasVipTier = this.payload?.vip_ticket_price !== undefined && this.payload?.vip_ticket_price !== null && String(this.payload?.vip_ticket_price).trim() !== "";
      ...
      if (!response.ok) {
        return this.makeResult(null, {
          message: payload?.message || payload?.error || "Failed to update event"
        });
      }
      return this.makeResult([mapBackendEvent(payload.event || {})], null);
    }
    if (this.operation === "delete") {
      ...
      if (!response.ok) {
        return this.makeResult(null, {
          message: payload?.message || payload?.error || "Failed to delete event"
        });
      }
      return this.makeResult([], null);
    }
  ...
  async runBookings() {
    ...
    if (!response.ok) {
      return this.makeResult([], {
        message: payload?.message || payload?.error || "Failed to fetch bookings"
      });
    }
    const rows = (payload.bookings || []).map(booking => mapBackendBooking(booking));
    const filtered = this.applyFilters(rows);
    if (this.headOnly) {
      return this.makeResult(null, null, filtered.length);
    }
...
const auth = {
  ...
  async signInWithOAuth(payload) {
    ...
    const pendingSignup = safeJsonParse(pendingSignupRaw ? decodeURIComponent(pendingSignupRaw) : null);
    ...
    const isMovieTeamFlow = movieTeamCookie === "true" || movieTeamFromRedirect || pendingSignup?.role === "organizer" || pendingSignup?.role === "movie_team";
        ...
        try {
          ...
          if (!session) {
            return {
              data: null,
              error: {
                message: "Your session has expired. Please sign in again and retry the upload."
              }
            };
          }
          ...
          if (!response.ok) {
            return {
              data: null,
              error: {
                message: payload?.message || payload?.error || "Failed to upload image"
              }
            };
          }
          return {
        ...
        try {
          ...
          if (!session) {
            return {
              data: null,
              error: {
                message: "Your session has expired. Please sign in again and retry the delete."
              }
            };
          }
          const normalizedPaths = paths.map(entry => extractUploadPath(entry)).filter(Boolean);
          ...
          if (!response.ok) {
            return {
              data: null,
              error: {
                message: payload?.message || payload?.error || "Failed to delete image"
              }
            };
          }
          return {
